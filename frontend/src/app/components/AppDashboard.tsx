...
export default function AppDashboard() {
  ...
  const filteredJobs = jobs
    .filter(job => {
      const matchesCategory = activeCategory === 'Todo' || job.category === activeCategory;
  ...
  const filteredServices = services
    .filter(service => {
      const matchesCategory = activeCategory === 'Todo' || service.tipo_de_oficio === activeCategory;
      const matchesSearch = service.oficio.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col relative">
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 pt-8 flex gap-6 lg:gap-8 pb-20">
        ...
        <aside className={`hidden md:block relative mt-50 mr-5 shrink-0 z-40 transition-all duration-300 ease-in-out ${isSidebarOpen ? 'w-64' : 'w-16'}`}>
          <div className="sticky top-28 bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
            ...
            <div className="flex items-center p-3 border-b border-gray-100/80">
              ...
              <span className={`ml-3 font-black text-xs text-gray-400 uppercase tracking-widest whitespace-nowrap transition-all duration-300 ${isSidebarOpen ? 'opacity-100' : 'opacity-0 w-0 hidden'
                }`}>
                Filtros
              </span>
            </div>
              ...
              {categories.map((cat) => {
                ...
                return (
                  <li key={cat.name}>
                    ...
                    >
                      <span className="text-xl shrink-0 w-8 flex justify-center">{cat.icon}</span>
                      <span className={`ml-3 font-semibold text-sm whitespace-nowrap overflow-hidden transition-all duration-300 text-left ${isSidebarOpen ? 'max-w-[150px] opacity-100' : 'max-w-0 opacity-0 ml-0'
                        }`}>
                        {cat.name}
                      </span>
                    </button>
        ...
        <div className={`md:hidden fixed top-0 left-0 h-full w-[280px] bg-white z-50 shadow-2xl transition-transform duration-300 flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
          }`}>
          <div className="flex items-center justify-between p-6 border-b border-gray-100">
            <span className="font-black text-gray-900 text-lg">Categorías</span>
            <button onClick={() => setIsSidebarOpen(false)} className="text-gray-400 hover:text-red-500 p-2 bg-gray-50 rounded-full cursor-pointer">
              <svg className="w-5 h-5 stroke-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
          </div>
          <ul className="p-4 space-y-2 overflow-y-auto">
            {categories.map((cat) => (
              <li key={`mob-${cat.name}`}>
                <button
                  onClick={() => { setActiveCategory(cat.name); setIsSidebarOpen(false); }}
                  className={`w-full flex items-center p-4 rounded-2xl gap-4 transition-all cursor-pointer ${activeCategory === cat.name ? 'bg-gray-900 text-white shadow-md' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'
                    }`}
                >
          ...
          <header className="mb-8">
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-10">
              <div>
                ...
                <p className="text-gray-500 mt-2 text-lg">¿Qué estás buscando hoy?</p>
              </div>
            </div>
          ...
          {searchQuery && (
            <div className="mb-6 flex items-center justify-between bg-blue-50/50 p-4 rounded-2xl border border-blue-100">
              <p className="text-sm font-medium text-blue-800">
                Resultados para: <span className="font-bold text-blue-900">"{searchQuery}"</span> en {activeCategory}
              </p>
              <button onClick={() => setSearchQuery('')} className="text-sm font-bold text-blue-600 hover:underline cursor-pointer">
                Limpiar
              </button>
            </div>
          )}
            ...
            <section className="animate-in fade-in duration-300">
              {loadingJobs ? (
                ...
                <div className="bg-white rounded-3xl border border-dashed border-gray-200 p-16 text-center max-w-xl mx-auto shadow-xs">
                  ...
                  <p className="text-gray-900 font-semibold text-lg">No hay resultados</p>
                  <p className="text-gray-500 text-sm mt-1">Intenta con otra categoría o cambia los términos de búsqueda.</p>
                  <button
                    ...
                    <Link key={job.id} href={`/trabajos/${job.id}`} className="group block cursor-pointer">
                      <div className="relative aspect-square mb-3.5 overflow-hidden rounded-2xl bg-gray-100 border border-gray-200/60 shadow-xs">
                        ...
                        <div className="absolute top-3 right-3 z-20">
                          <button
                            className="w-8 h-8 bg-white/90 backdrop-blur-md rounded-full flex items-center justify-center shadow-sm text-gray-400 hover:text-red-500 hover:scale-105 transition-all cursor-pointer"
                            onClick={(e) => e.preventDefault()}
                          >
                            <svg className="w-4 h-4 transition-colors duration-200" fill="currentColor" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" /></svg>
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="space-y-1 px-1">
                        <div className="flex justify-between items-start gap-2">
                          <h3 className="font-bold text-gray-900 leading-snug truncate flex-1" title={job.title}>{job.title}</h3>
                          <div className="flex items-center gap-1 shrink-0 text-sm font-semibold text-gray-900">
                            <span>⭐</span>
                            <span>{job.perfiles?.rating_promedio ? Number(job.perfiles.rating_promedio).toFixed(1) : 'N/A'}</span>
                          </div>
                        </div>
                        ...
                        {job.distancia_metros !== undefined && job.distancia_metros !== null ? (
                          <p className="text-sm text-blue-600 font-semibold">📍 A {formatDistance(job.distancia_metros)}</p>
                        ) : (
                          <p className="text-sm text-gray-400">Sin ubicación exacta</p>
                        )}

                        <p className="text-base font-black text-gray-900 pt-1">${job.budget || 0} USD</p>
                      </div>
                    </Link>
                  ))}
                </div>
                ...
                <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  ...
                  {filteredServices.map((servicio) => (
                    <Link key={servicio.id} href={`/servicios/${servicio.id}`} className="group block cursor-pointer">
                      ...
                      <div className="space-y-1 px-1">
                        <div className="flex justify-between items-start gap-2">
                          <h3 className="font-bold text-gray-900 leading-snug truncate flex-1" title={servicio.oficio}>{servicio.oficio}</h3>
                          <div className="flex items-center gap-1 shrink-0 text-sm font-semibold text-gray-900">
                            <span>⭐</span>
                            <span>{servicio.perfiles?.rating_promedio ? Number(servicio.perfiles.rating_promedio).toFixed(1) : 'N/A'}</span>
                          </div>
                        </div>

                        <p className="text-xs font-semibold text-gray-500 uppercase">{servicio.tipo_de_oficio}</p>

                        {servicio.distancia_metros !== undefined && servicio.distancia_metros !== null ? (
                          <p className="text-sm text-blue-600 font-semibold">📍 A {formatDistance(servicio.distancia_metros)}</p>
                        ) : (
                          <p className="text-sm text-gray-400">Sin ubicación exacta</p>
                        )}

                        {/* Ojo aquí: Si tus servicios manejan "precio" en vez de "budget", cámbialo en esta línea */}
                        <p className="text-base font-black text-gray-900 pt-1">${servicio.budget || 0} USD</p>
                      </div>
                    </Link>
                  ))}
                </div>
